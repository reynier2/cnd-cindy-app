...
# --- 🏗️ ENGINEERING COMPLIANCE CORE (Manning's Eq & Soil Bearing) ---
...
# --- 📡 ADOPTION ENGINE (silent usage pings with location) ---
...
IDENTITY RULES:
- NEVER invent human names (no "Henrik", no signatures). Only use "CND Real Estate Services".
{client_rule}

TASK: Analyze ALL photos. Create ONE itemized bid.
MATERIAL RULES: Name Brand + Product + Size (e.g. Quikrete 80lb). Show math (15 x $6 = $90). List equivalents.
ENGINEERING RULES:
- If pipe/culvert seen: Estimate Diameter & Slope. Run Manning's Eq (n=0.013). Stamp PASS/RED FLAG.
- If foundation seen: Check soil bearing (Limit 30 PSI). Stamp PASS/RED FLAG.
WATER RULES: Mention frost line depth, PRV if >80psi, lead/poly flags.
...
# --- 📸 PHOTO GALLERY GENERATOR (For PDF) ---
...
# --- 📄 PDF BUILDER (with Photos + Fallback) ---
...
<div class="header"><h1>CND REAL ESTATE SERVICES</h1>
<p style="color:#666;font-style:italic;margin:5px 0 0 0;">{subtext}</p>{prepared}</div>
{photo_gallery_html}
